...
public final class FloodFill {
  ...
  public void setPatternImage( Image img){
    int width = img.getWidth(null); 
    int height = img.getHeight(null); 
    this.patternImage = new BufferedImage(width, height, BufferedImage.TYPE_INT_ARGB);
    Graphics g = this.patternImage.getGraphics();
  ...
  public void fill(int x, int y, Color color) {
    ...
    System.out.println("START="+this.startColor);
    System.out.println("BLACK="+Color.BLACK.getRGB());
    
    if ((fillColor != this.startColor) && (this.startColor!=Color.BLACK.getRGB())) {        
  ...
  public void fill(int x, int y, Pattern pattern) {
	  this.pattern = pattern;
	  fill( x, y, new Color(pattern.getRGB(0,0)));
  }
  ...
  public void fill(int x, int y, Color color, Color patternColor, int patternWidth, int patternHeight) {
    ...
    if ((this.maskColor != this.startColor) || (this.startColor!=Color.BLACK.getRGB())) {
        ...
        while (x <= right) {
          if (maskImage[yOff + x] == startColor) {
            ...
            if (antialiased) {
              ...
              antialiasedColor = antialiasedColor + ((fillColor(x,y-1) >> 1) & 0x7F7F7F7F);      
  ...
  private int floodFill(int x, int y) { 
    ...
    do {
      ...
      maskImage[yOff + left] = maskColor; //maskColor;
      left--;
    } 
    ...
    if ((antialiased) && (left >= 0)) {
      ...
      antialiasedColor = (antialiasedColor >> 1) & 0x7F7F7F7F;
      antialiasedColor = antialiasedColor + ((fillColor(x,y) >> 1) & 0x7F7F7F7F);      
      if (antialiasedColor != startColor) {
    ...
    do {
      ...
      maskImage[yOff + right] = maskColor; //maskColor;
      right++;
    } 
